...
covid_model = load_model('models/covidCNN.h5')
braintumor_model = load_model('models/braintumor.h5')
diabetes_model = joblib.load('models/diabetes_KNN_model.pkl')
heart_model = joblib.load('models/heart_RFC_model.pkl')
...
def resultd():
    if request.method == 'POST':
        ...
        pred = diabetes_model.predict([[glucose, bmi, age]])
...
def resulth():
    if request.method == 'POST':
        ...
        age = float(request.form['age'])
        print(np.array([[sts, eia, op, mhra, tcp]]).reshape(1, -1))
        pred = heart_model.predict([np.array([sts, eia, op, mhra, tcp])])
        #pred = heart_model.predict([[sts, eia, op, mhra, tcp]])
        # pb.push_sms(pb.devices[0],str(phone), 'Hello {},\nYour Diabetes test results are ready.\nRESULT: {}'.format(firstname,['NEGATIVE','POSITIVE'][pred]))
